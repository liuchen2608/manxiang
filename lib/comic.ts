import {americanComicPrompt} from "./american-comic";
import {watercolorPrompt} from "./watercolor-picture-book";
import {blackWhitePrompt} from "./black-white-comic";
import {japaneseColorPrompt} from "./japanese-color-comic";
import { z } from "zod";
...
export const imageResponseSchema=z.object({image:z.string().max(12_000_000).regex(/^data:image\/(?:png|jpeg|webp);base64,[A-Za-z0-9+/]+={0,2}$/)});
...
export function imagePrompt(project:Pick<Project,"style"|"characters">, panel?:Panel){
 if(project.style==="美式漫画")return americanComicPrompt(project.characters,panel).prompt;
 if(project.style==="水彩绘本")return watercolorPrompt(project.characters,panel).prompt;
 if(project.style==="黑白漫画")return blackWhitePrompt(project.characters,panel).prompt;
 if(project.style==="日系彩漫")return japaneseColorPrompt(project.characters,panel).prompt;
...
export function changeProjectStyle(project:Project,style:string):Project {
 if(project.style===style)return project;
 return {...project,style,referenceStale:!!project.reference,panels:project.panels.map(panel=>({...panel,stale:!!panel.image}))};
}
export function imageSize(style:string){return style==="日系彩漫"?"1024x1024":"1536x1024";}
