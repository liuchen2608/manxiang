import templates from '../skills/japanese-color-comic/templates.json';
import type {Character,Panel} from './comic';
function fill(template:string,values:Record<string,string>){
 return template.replace(/\{\{(\w+)\}\}/g,(_match,key:string)=>{
  if(!(key in values))throw new Error('日系彩漫模板字段缺失：'+key);
  return values[key];
 });
}
export function japaneseColorPrompt(characters:Character[],panel?:Panel){
