import templates from '../skills/black-white-comic/templates.json';
import type {Character,Panel} from './comic';
function fill(template:string,values:Record<string,string>){
 return template.replace(/\{\{(\w+)\}\}/g,(_match,key:string)=>{
  if(!(key in values))throw new Error('黑白漫画模板字段缺失：'+key);
  return values[key];
 });
}
export function blackWhitePrompt(characters:Character[],panel?:Panel){
